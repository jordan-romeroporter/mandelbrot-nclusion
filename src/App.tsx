...
import { Viewport, ViewPreset } from './types';
...
function getRandomStartingView(): Viewport {
  const view = STARTING_VIEWS[Math.floor(Math.random() * STARTING_VIEWS.length)];
  return {
...
function App() {
  ...
  const handleViewportChange = useCallback((preset: ViewPreset) => {
    if (!isCalculating) {
      setViewport({
