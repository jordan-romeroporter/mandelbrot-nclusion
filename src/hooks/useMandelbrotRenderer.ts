...
export function useMandelbrotRenderer({
  ...
  const { workers, isInitialized, error: workerError } = useWebWorkers({ 
    enabled: useWorkers 
  });

  const renderWithWorkers = useCallback(async (canvas: HTMLCanvasElement) => {
    ...
    onComplete?.();
  }, [workers, size, colorScheme, maxIterations, viewport, onProgress, onComplete]);

  const renderSynchronous = useCallback(async (canvas: HTMLCanvasElement) => {
    const ctx = canvas.getContext('2d');
    if (!ctx) return;
